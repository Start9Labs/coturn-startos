...
**The allowed entry is what keeps that list from blocking the server from itself.** `external-ip` registers a two-way mapping, and coturn applies the public → private direction to every address attribute it decodes — the `XOR-PEER-ADDRESS` of a `CreatePermission` included. So when two clients on this server exchange relay candidates, the peer address each one sends is the public IP, which coturn rewrites to the container address _before_ testing it against the denied ranges. That address is inside `10.0.0.0-10.255.255.255`, so the permission is refused and the relay-to-relay candidate pair — the one two clients behind symmetric NAT depend on — never comes up. `good_peer_addr` consults the allowed list first and returns on a match, so naming that single host restores it while the LAN and the other containers stay denied.

It is IPv4 only. Coturn refuses unique-local peers ahead of the allowed list, so an IPv6 allocation's relay candidate — a container address in `fd00::/8` — cannot be rescued the same way; relay-to-relay over IPv6 on one server does not work.
