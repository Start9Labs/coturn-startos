...
- **Run `turnutils_uclient` with `--entrypoint`.** The upstream image's `docker-entrypoint.sh` evals its arguments, so the client ends up dialing localhost and reporting `Connection refused` against a server that is fine. A failed login reads `Cannot complete Allocation`. Under `-y` (client-to-client: two allocations relaying to each other) **the channel bind must succeed** — `channel bind: error 403` there means `denied-peer-ip` refused Coturn's own container address, which is what the other allocation's relay candidate resolves to, so the `allowed-peer-ip` entry has gone missing. It was once recorded here as the success case; it is the relay-to-relay bug. No Docker is needed — attach and read the secret in place: `start-cli package attach coturn -n coturn-sub -- sh -c 'S=$(grep -m1 "^static-auth-secret=" /var/lib/coturn/turnserver.conf | cut -d= -f2); turnutils_uclient -t -c -y -n 1 -p 3478 -W "$S" 127.0.0.1'`.
...
- **Only what the rendered config depends on belongs in `main`'s `.const()`** — the realm, the public IPs, the container's own address, the secret, and the static account. Per-address enable/disable feeds the health checks and is watched with `.onChange` into a local instead — capturing it in the `.const()` would restart `turnserver` on every toggle. If you add an input that the rendered config depends on, it goes in the `.const()`; anything the checks alone read does not.
